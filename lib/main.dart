...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  Database? _db;
  Map<String, Map<String, dynamic>> daysData = {}; // date -> {'total':..., 'night':..., 'timestamps': [...]}
  ...
  Future<void> _refreshCounts() async {
    ...
    for (var row in res) {
      ...
      data.putIfAbsent(dayKey, () => {'total': 0, 'night': 0, 'timestamps': <DateTime>[]});
      data[dayKey]!['total'] += 1;
      data[dayKey]!['timestamps'].add(dt);

      // Comptage nuit 22h → 8h
      final nightStart = DateTime(dt.year, dt.month, dt.day, 22);
      final nightEnd = nightStart.add(const Duration(hours: 10)); // 22h → 8h du lendemain
      if (dt.isAfter(nightStart) || dt.isBefore(nightStart.subtract(const Duration(hours: 14)))) {
        data[dayKey]!['night'] += 1;
      }
    }
    ...
    setState(() {
      // Inverse chronologique des jours
      daysData = Map.fromEntries(data.entries.toList().reversed);
    });
  }
  ...
  Future<void> exportJsonl() async {
    ...
    final dir = await Directory.systemTemp.createTemp();
    final file = File('${dir.path}/tracker_events.jsonl');
    final sink = file.openWrite();
    ...
    await Share.shareXFiles([XFile(file.path)], text: 'Export Tracker Events');
  }
              ...
              final confirm = await showDialog<bool>(
                ...
                builder: (BuildContext context) {
                  return AlertDialog(
                    ...
                    content: const Text('Effacer toutes les entrées de la base ?'),
                    actions: [
      ...
      body: Padding(
        ...
        child: ListView(
          children: daysData.entries.map((entry) {
            ...
            final timestamps = info['timestamps'] as List<DateTime>;

            // ordre inverse des heures
            final reversedTimestamps = timestamps.reversed.toList();
            ...
            return Column(
              ...
              children: [
                const Divider(),
                Text(day, style: const TextStyle(fontWeight: FontWeight.bold)),
                Text('Total journée (8h-8h): ${info['total']}'),
                Text('Total nuit (22h-8h): ${info['night']}'),
                ...reversedTimestamps.map((dt) => Text(
